...
// return because an Err means it cannot be handled ? is almost1 exactly equivalent
// to an unwrap which returns instead of panicking on Errs.
...
// at older code. The same multiply function from the previous example would
// look like this using try!:
