...
// Recall that Rust allows us to create aliases. Conveniently, we can define one
// for the specific Result in question.
//
// At a module level, creating aliases can be particularly helpful.
// Errors found in a specific module often have the same Err type, so a single
// alias can succinctly define all associated Results.
