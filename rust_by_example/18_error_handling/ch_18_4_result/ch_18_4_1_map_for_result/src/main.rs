...
// Generally, we want to return the error to the caller so it can decide
...
fn main() {
    ...
    // A much more helpful error message (with `and_then()`)
    let twenty = multiply_with_and_then("10", "2");
    print(twenty);
    let tt = multiply_with_and_then("t", "2");
    print(tt);
}
