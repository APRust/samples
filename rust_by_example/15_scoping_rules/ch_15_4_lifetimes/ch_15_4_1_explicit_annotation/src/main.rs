...
// references should be valid. In cases where lifetimes are not elided1,
...
// `print_refs` takes two reference to `i32' which have different lifetimes `'a` and `'b`.
...
fn main() {
    ...
    failed_borrow();
    // `failed_borrow` contains no reference to force `'a` to be
