...
// they have excess space. You can create a HashMap with a certain starting capacity
// using HashMap::with_capacity(uint), or use HashMap::new() to get a HashMap
...
fn main() {
    ...
    for (contacts, &number) in contacts.iter() {
        println!("Calling {}: {}", contacts, call(number));
    }
}
