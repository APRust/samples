...
// (allocated on the heap) by creating a Box<T>. A box is a smart pointer to a heap
// allocated value of type T. When a box goes out of scope, its destructor is called,
...
// A Rectangle can be specified by where its top left and bottom right corners
// are in space
#[allow(dead_code)]
struct Rectangle {
    top_left: Point,
