...
// Rc keeps track of the number of the references which means the number of owners of
...
// When an Rc's reference count becomes zero (which means there are no remaining owners),
...
fn main() {
    ...
    // Error! `rc_example` already moved into `rc_a`
