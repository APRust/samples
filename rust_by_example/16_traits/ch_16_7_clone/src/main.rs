...
// assignments or function calls. However, sometimes we need to make a copy
...
// A tuple struct with resources that implement the `Clone` trait
#[derive(Clone, Debug)]
...
fn main() {
    // Instantiate Unit
    let unit = Unit;
    // Copy `Unit`, there are no resource to move
    ...
    // Clone `moved_pair` into `cloned_pair` (resource are included)
