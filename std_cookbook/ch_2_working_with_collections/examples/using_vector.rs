...
fn main() {
    ...
    println!("fruits after insertions: {:?}", fruits);

    // Swap two elements
    fruits.swap(0, 1);
    println!("fruits after swap: {:?}", fruits);

    /// Access single element in a vector
    ...
    // Only keeps names starting with 'A'
    ...
    nums.dedup();
    // Doesn't print what you might expect
    ...
    print!("The first two letter of the alphabet are: ");
    ...
    /// If you are working very bib datasets, you can optimize the performance of your vector:
    ...
    // Shrink the vector as closes as possible to its length
    large_vec.shrink_to_fit();
    println!("large_vec after shrink: ");
    println!("len: \t\t{}", large_vec.len());
    println!("capacity: \t{}", large_vec.capacity());
